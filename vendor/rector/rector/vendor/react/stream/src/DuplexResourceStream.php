...
final class DuplexResourceStream extends EventEmitter implements DuplexStreamInterface
{
    ...
    public function __construct($stream, $loop = null, $readChunkSize = null, $buffer = null)
    {
        ...
        $this->buffer = $buffer;
        $that = $this;
        $this->buffer->on('error', function ($error) use($that): void {
            $that->emit('error', array($error));
        });
        $this->buffer->on('close', array($this, 'close'));
        $this->buffer->on('drain', function () use($that): void {
            $that->emit('drain');
        });
        $this->resume();
    }
    ...
    {
        $error = null;
        \set_error_handler(function ($errno, $errstr, $errfile, $errline) use(&$error): void {
