...
namespace RectorPrefix202503\React\ChildProcess;
...
use RectorPrefix202503\React\Stream\ReadableResourceStream;
use RectorPrefix202503\React\Stream\ReadableStreamInterface;
...
class Process extends EventEmitter
{
    ...
    public function start($loop = null, $interval = 0.1)
    {
        ...
        $errstr = '';
        \set_error_handler(function ($_, $error) use(&$errstr): void {
        ...
        $that = $this;
        $closeCount = 0;
        $streamCloseHandler = function () use(&$closeCount, $loop, $interval, $that): void {
            $closeCount--;
            if ($closeCount > 0) {
                return;
            }
            ...
            $loop->addPeriodicTimer($interval, function ($timer) use($that, $loop): void {
                if (!$that->isRunning()) {
