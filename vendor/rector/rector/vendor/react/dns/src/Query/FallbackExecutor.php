...
final class FallbackExecutor implements ExecutorInterface
{
    ...
    public function query(Query $query)
    {
        ...
        return new Promise(function ($resolve, $reject) use(&$promise, $fallback, $query, &$cancelled): void {
            $promise->then($resolve, function (\Exception $e1) use($fallback, $query, $resolve, $reject, &$cancelled, &$promise): void {
                ...
                $promise = $fallback->query($query)->then($resolve, function (\Exception $e2) use($e1, $reject): void {
        ...
        }, function () use(&$promise, &$cancelled): void {
