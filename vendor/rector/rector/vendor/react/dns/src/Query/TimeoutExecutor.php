...
final class TimeoutExecutor implements ExecutorInterface
{
    ...
    public function query(Query $query)
    {
        ...
        $loop = $this->loop;
        $time = $this->timeout;
        return new Promise(function ($resolve, $reject) use($loop, $time, $promise, $query): void {
            $timer = null;
            $promise = $promise->then(function ($v) use(&$timer, $loop, $resolve): void {
                if ($timer) {
                    $loop->cancelTimer($timer);
                }
                $timer = \false;
                $resolve($v);
            }, function ($v) use(&$timer, $loop, $reject): void {
                if ($timer) {
                    $loop->cancelTimer($timer);
                }
                $timer = \false;
            ...
            $timer = $loop->addTimer($time, function () use($time, &$promise, $reject, $query): void {
