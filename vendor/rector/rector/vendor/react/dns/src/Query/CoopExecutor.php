...
final class CoopExecutor implements ExecutorInterface
{
    ...
    public function query(Query $query)
    {
        ...
        if (isset($this->pending[$key])) {
            ...
            $counts =& $this->counts;
            $promise->then(function () use($key, &$pending, &$counts): void {
                unset($pending[$key], $counts[$key]);
            }, function () use($key, &$pending, &$counts): void {
                unset($pending[$key], $counts[$key]);
            });
        }
        ...
        $pending =& $this->pending;
        $counts =& $this->counts;
        return new Promise(function ($resolve, $reject) use($promise): void {
            $promise->then($resolve, $reject);
        }, function () use(&$promise, $key, $query, &$pending, &$counts): void {
            if (--$counts[$key] < 1) {
                unset($pending[$key], $counts[$key]);
                $promise->cancel();
                $promise = null;
