...
final class HappyEyeBallsConnectionBuilder
{
    ...
    {
        $that = $this;
        return new Promise\Promise(function ($resolve, $reject) use($that): void {
            $lookupResolve = function ($type) use($that, $resolve, $reject) {
                return function (array $ips) use($that, $type, $resolve, $reject): void {
            ...
            $that->resolverPromises[Message::TYPE_A] = $that->resolve(Message::TYPE_A, $reject)->then(function (array $ips) use($that) {
                ...
                $deferred = new Promise\Deferred(function () use(&$ips): void {
                    // discard all IPv4 addresses if cancelled
                    $ips = array();
                });
                $timer = $that->loop->addTimer($that::RESOLUTION_DELAY, function () use($deferred, $ips): void {
                    $deferred->resolve($ips);
                });
                $that->resolverPromises[Message::TYPE_AAAA]->then(function () use($that, $timer, $deferred, &$ips): void {
                    $that->loop->cancelTimer($timer);
                    $deferred->resolve($ips);
                });
                return $deferred->promise();
            })->then($lookupResolve(Message::TYPE_A));
        }, function ($_, $reject) use($that): void {
    ...
    public function check($resolve, $reject)
    {
        ...
        $that = $this;
        $that->connectionPromises[$index]->then(function ($connection) use($that, $index, $resolve): void {
            unset($that->connectionPromises[$index]);
            $that->cleanUp();
            $resolve($connection);
        }, function (\Exception $e) use($that, $index, $ip, $resolve, $reject): void {
            unset($that->connectionPromises[$index]);
            $that->failureCount++;
        ...
        if ($this->nextAttemptTimer === null && (\count($this->connectQueue) > 0 || $this->resolved[Message::TYPE_A] === \false || $this->resolved[Message::TYPE_AAAA] === \false)) {
            $this->nextAttemptTimer = $this->loop->addTimer(self::CONNECTION_ATTEMPT_DELAY, function () use($that, $resolve, $reject): void {
                $that->nextAttemptTimer = null;
                if ($that->connectQueue) {
                    $that->check($resolve, $reject);
                }
