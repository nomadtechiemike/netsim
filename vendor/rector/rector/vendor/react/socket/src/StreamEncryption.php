...
class StreamEncryption
{
    ...
    public function toggle(Connection $stream, $toggle)
    {
        ...
        $that = $this;
        $toggleCrypto = function () use($socket, $deferred, $toggle, $method, $that): void {
        ...
        return $deferred->promise()->then(function () use($stream, $socket, $loop, $toggle) {
            ...
            $stream->resume();
            return $stream;
        }, function ($error) use($stream, $socket, $loop): void {
            $loop->removeReadStream($socket);
            $stream->resume();
            throw $error;
        });
    ...
    {
        $error = null;
        \set_error_handler(function ($_, $errstr) use(&$error): void {
