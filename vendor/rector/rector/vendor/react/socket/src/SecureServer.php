...
final class SecureServer extends EventEmitter implements ServerInterface
{
    ...
    public function __construct(ServerInterface $tcp, $loop = null, array $context = array())
    {
        ...
        $that = $this;
        $this->tcp->on('connection', function ($connection) use($that): void {
            $that->handleConnection($connection);
        });
        $this->tcp->on('error', function ($error) use($that): void {
    ...
    public function handleConnection(ConnectionInterface $connection)
    {
        ...
        $that = $this;
        $this->encryption->enable($connection)->then(function ($conn) use($that): void {
