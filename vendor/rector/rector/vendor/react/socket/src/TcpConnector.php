...
final class TcpConnector implements ConnectorInterface
{
    ...
    public function connect($uri)
    {
        ...
        // wait for connection
        $loop = $this->loop;
        return new Promise\Promise(function ($resolve, $reject) use($loop, $stream, $uri): void {
            $loop->addWriteStream($stream, function ($stream) use($loop, $resolve, $reject, $uri): void {
                ...
                if (\false === \stream_socket_get_name($stream, \true)) {
                    ...
                    if (\function_exists('socket_import_stream')) {
                        ...
                        $errno = 0;
                        $errstr = '';
                        \set_error_handler(function ($_, $error) use(&$errno, &$errstr): void {
        ...
        }, function () use($loop, $stream, $uri): void {
            $loop->removeWriteStream($stream);
            \fclose($stream);
            // @codeCoverageIgnoreStart
